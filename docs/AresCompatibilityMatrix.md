...
| Ares EMP duration/cap state | To be confirmed by target scan | yes | partial | `AresEMP.test.ts` | [Ares EMP docs](https://ares-developers.github.io/Ares-docs/restored/emp.html), [Antares EMPulse](https://github.com/Phobos-developers/Antares/blob/419626d/src/Misc/EMPulse.cpp) | Per-Techno counters implement positive stacking/capping, uncapped legacy mode, set-unless-longer mode, negative removal, `EMP.Modifier`, `ImmuneToEMP`, default immunity, veteran `EMPIMMUNE`, movement/attack paralysis, powered blackout and aircraft crash entry. | Manager suspension (factory/spawner/slave work), sparkle presentation, mission preservation, and full building subsystem notifications still need integration fixtures. |
