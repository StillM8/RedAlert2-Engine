...
export class CrashableTrait {
    ...
    [NotifyTick.onTick](target: any, context: any): void {
        ...
        // The previous short-circuited OR expression never reached the branch
        // that set crashingEvtSent, so NotifyCrash/ObjectCrashingEvent could not
        // fire at all. Emit them exactly once at crash start and include the
        // attacker context so survivor failures retain kill attribution.
        if (!this.crashingEvtSent) {
            this.crashingEvtSent = true;
            target.traits
                .filter(NotifyCrash)
                .forEach((trait) => trait[NotifyCrash.onCrash](target, context, this.attackerInfo));
