...
const OVERRIDES: Readonly<Record<string, CapabilityOverride>> = {
    ...
    "new/attacheffect.rst": {
        ...
        tests: ["AresAttachEffect.test.ts", "AresAttachEffectRuntime.test.ts", "AresAttachEffectTraitBridge.test.ts", "AresAttachEffectObjectFactory.test.ts"],
        dependencies: ["ares.effective-ini"], targetModUsage: "required",
        notes: "AttachEffect fields for TechnoTypes and Warheads are parsed and normalized; ObjectFactory registration and the generic trait cover automatic TechnoType spawn timing, renewal, protection retry, active instances, reapplication/stacking decisions, expiry, entry discard, and aggregate numeric modifiers. Animation/cloak/combat application, transport/temporal lifecycle, save/load, and deterministic multiplayer replay remain open.",
    },
    "new/custombuildingfoundations.rst": {
