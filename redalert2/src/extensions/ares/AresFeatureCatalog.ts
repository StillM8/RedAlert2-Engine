...
const DOCUMENTED_KEYS: Readonly<Record<string, readonly string[]>> = {
    ...
    "new/superweapons.rst": ["SW.FireIntoShroud", "SW.AutoFire", "SW.ManualFire", "SW.Range", "SW.CreateRadarEvent", "SW.Deferment", "SW.PostDependent", "SW.UseAITargeting", "SW.AITargeting", "SW.AITargeting.Constraints", "SW.AITargeting.Preference", "SW.AIRequiresTarget", "SW.AIRequiresHouse", "SW.ChargeToDrainRatio", "SW.Unstoppable", "SW.InitialReady", "SW.VirtualCharge", "Money.Amount", "Money.DrainAmount", "Money.DrainDelay", "SW.Animation", "EVA.*", "Message.*", "Text.*", "Light.*", "Lightning.*", "Nuke.*", "Dominator.*", "Chronosphere.*", "Protect.*", "Mutate.*", "ParaDrop.*", "SpyPlane.*", "SonarPulse.Delay", "Deliver.Types", "DropPod.Types", "DropPod.Minimum", "DropPod.Maximum", "DropPod.Veterancy", "DropPodWeapon", "DropPodTrailer", "EMPulse.Linked", "EMPulse.TargetSelf", "EMPulse.PulseDelay", "EMPulse.PulseBall", "EMPulse.Cannons", "EMPulseCannon"],
...
const OVERRIDES: Readonly<Record<string, CapabilityOverride>> = {
    ...
    "new/superweapons.rst": {
        ...
        notes: "Common fields, SW.Range for supported ranged effects, SW.Deferment for supported state machines, Money.Amount launch transactions, manual SW.RequiresTarget/SW.RequiresHouse gates, InitialReady/VirtualCharge timer state, PostDependent stage selection, common radar-event delivery, target filters, GenericWarhead, UnitDelivery, SonarPulse, the EMPulse launch state, and the Antares DropPod core placement handler are present; Money.DrainAmount/DrainDelay, animation/presentation, AI, persistence, and the remaining handlers are not complete.",
    },
    "restored/emp.rst": {
