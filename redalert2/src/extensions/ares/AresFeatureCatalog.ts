...
const DOCUMENTED_KEYS: Readonly<Record<string, readonly string[]>> = {
    ...
    "new/superweapons.rst": ["SW.Range", "SW.CreateRadarEvent", "SW.Deferment", "SW.PostDependent", "SW.AITargeting", "SW.ChargeToDrainRatio", "SW.Unstoppable", "Money.Amount", "SW.Animation", "EVA.*", "Message.*", "Text.*", "Light.*", "Lightning.*", "Nuke.*", "Dominator.*", "Chronosphere.*", "Protect.*", "Mutate.*", "ParaDrop.*", "SpyPlane.*", "SonarPulse.Delay", "Deliver.Types"],
...
const OVERRIDES: Readonly<Record<string, CapabilityOverride>> = {
    ...
    "new/superweapons.rst": {
        ...
        notes: "Common fields, target filters, GenericWarhead, UnitDelivery, and SonarPulse are present; the documented surface and remaining handlers are not complete.",
    },
    "restored/emp.rst": {
