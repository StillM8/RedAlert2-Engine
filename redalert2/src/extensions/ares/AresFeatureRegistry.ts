...
export const DEFAULT_ARES_FEATURES: readonly ExtensionFeature[] = [
    ...
    },
    {
        ...
        parserImplemented: true,
        runtimeImplemented: true,
        tests: ["AresVehicleThief.test.ts", "AresSurvivorRuntime.test.ts"],
        notes: "VehicleThief and CanDrive share a data-defined hijack action path. Enemy hijacks, neutral DriverKilled reclaim, TechnoType/Country CanBeDriven gates, mind-control cleanup, absorbed-hijacker recovery/recycle state, target/task integration, and survivor pilot-count adjustment are wired; building hijacks, sounds, trigger/event parity, and full save/network integration remain open.",
    },
    {
        id: "ares.emp",
    ...
    },
    {
        ...
        description: "Ares Gattling.Cycle stage wrap behavior on the existing gattling state machine",
        implemented: false,
        parserImplemented: true,
        runtimeImplemented: true,
        tests: ["AresGattlingCycle.test.ts"],
        notes: "Gattling.Cycle is parsed from TechnoType rules and wraps the deterministic gattling accumulator back to stage zero instead of pinning at the final stage. Save/load of mid-cycle gattling state and multiplayer/target-content certification remain open; this entry does not claim a second generic staged-weapon subsystem.",
    },
    {
    ...
    },
    {
        ...
        description: "Ares specific passengers, capacity, manual-control, initial-payload, building-absorb, and promotion semantics",
        implemented: false,
        parserImplemented: true,
        runtimeImplemented: true,
    ...
    {
        id: "ares.survivors",
        description: "Ares vehicle/aircraft pilot and transport-passenger survivor semantics",
        implemented: false,
        parserImplemented: true,
        runtimeImplemented: true,
        tests: ["AresSurvivors.test.ts", "AresSurvivorRuntime.test.ts", "CrashableLifecycle.test.ts"],
        notes: "Survivor.PilotCount, rank-specific pilot/passenger chances, Survivor.Side#, Ares percentage normalization, 50%-health pilot spawning, inherited pilot veterancy/XP, explicit airborne passenger paradrops, crash-time resolution, and VehicleThief pilot-count adjustment are wired. Positive-chance failed/blocked pilot virtual kill credit, save/load, multiplayer certification, and real target-content verification remain open.",
    },
    {
